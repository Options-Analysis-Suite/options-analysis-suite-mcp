...
export function register(server: McpServer, client: ProxyClient): void {
  server.registerTool(
    ...
    {
      title: 'IV History',
      description: 'Get historical implied volatility (IV) and historical volatility (HV) for a stock or ETF. Shows how option-implied expected moves and realized moves have evolved. High IV relative to HV suggests options are expensive; low IV relative to HV suggests options are cheap. Large windows return a compact recent/trend summary by default.' + HISTORY_PROVENANCE_TIMES,
      inputSchema: {
