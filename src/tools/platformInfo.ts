...
const LIVE_CAPABILITY =
  ...
  + "\n- LIVE options chains (Pro and above): get_live_options_chain fetches a real-time chain from the broker credential stored on the user's account. Every other option-CHAIN tool here is end-of-day, from the last completed session (get_regime with scope=\"intraday\" does expose intraday exposure, but not chain prices). It spends the user's own broker quota and is capped at 10 requests/minute, so prefer it when the question is about current or intraday prices, and the end-of-day tools otherwise."
  ...
  + "\n- Model calibration fits (Pro and above): get_regime_fits returns the calibrated parameters and fit quality (IV and price RMSE) for the eight pricing models on a symbol, with an error history, for the regime universe of about 124 symbols; a symbol outside it returns an empty history. Distinct from get_regime, which reports which market regime a symbol is in rather than how well a model fits it."
  + "\n- EOD dealer positioning (no Pro requirement): get_dealer_positioning returns last-close net GEX/DEX over 0-60 days, the dealer regime, the gamma flip, call and put walls, the gamma magnet, the 30-day expected move (a decimal fraction of spot, and in dollars) and the top contributing strikes for roughly 5,500 listed equities and ETFs. Its gamma flip is a coarse-grid level with no search status; it is the last-close claim, and get_live_dealer_positioning is the live one - neither substitutes for the other."
