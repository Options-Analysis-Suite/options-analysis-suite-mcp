...
const LIVE_CAPABILITY =
  ...
  + "\n- LIVE dealer positioning (Pro and above): get_live_dealer_positioning computes net GEX/DEX plus vega, vanna, charm and vomma, the gamma flip, call and put walls, gamma concentration and a positive/negative gamma regime, in real time from the user's own broker chain over the nearest four expirations, or over one named expiration (a same-day book, for instance). Metric coverage identifies partial sums and unavailable observations; levels require complete coverage of their inputs. It is the only tool that can answer 'where is the gamma flip right now'. Each call costs five weighted units against the 10-unit/min live-broker limit, two when it names one expiration; actual broker requests vary with provider and cache state, so it is not for looping over symbols."
